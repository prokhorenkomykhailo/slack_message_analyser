#!/usr/bin/env python3
"""
Create Final Step 2 Analysis Using phase4_clusters_refined.json as Benchmark
Uses the exact same formulas as Step 1 but with correct Step 2 benchmark and model data
"""
...
def load_final_step2_data():
    """Load final Step 2 data for analysis"""
    
    print("Loading Step 2 benchmark from phase4_clusters_refined.json...")
    # Load Step 2 benchmark clusters (15 clusters from phase4_clusters_refined.json)
    ...
    print(f"✅ Loaded {len(benchmark_clusters)} benchmark clusters from phase4_clusters_refined.json")
...
def create_final_step2_analysis():
    """Create final Step 2 analysis using phase4_clusters_refined.json as benchmark"""
    
    print("🎯 Creating Final Step 2 Analysis with phase4_clusters_refined.json as Benchmark")
    print("=" * 80)
    
    # Load correct data
    benchmark_clusters, model_data = load_final_step2_data()
    ...
    ws.cell(row=current_row, column=1, value="SHOWING STEP 2 REFINED CLUSTERS PERFORMANCE WITH ACCURATE BENCHMARK DATA")
    ws.cell(row=current_row, column=1).font = title_font
    ws.cell(row=current_row, column=1).fill = green_fill
    ws.merge_cells(f'A{current_row}:M{current_row}')
    current_row += 2
    
    # Summary counts (using correct Step 2 data)
    cluster_titles = model_data.get('cluster_titles', [])
    benchmark_eval = model_data.get('benchmark_evaluation', {})
    num_benchmark = len(benchmark_clusters)  # 15 clusters from phase4_clusters_refined.json
    ...
    # Calculate metrics using correct Step 2 data
    ...
    performance_data = [
        ['Total Benchmark Messages', total_benchmark_messages, 'Sum of phase4_clusters_refined.json counts', 'Ground truth message count for Step 2'],
    ...
    # Create topic detail rows for all 15 benchmark clusters from phase4_clusters_refined.json
    ...
    # Individual topic performance for all 15 benchmark clusters from phase4_clusters_refined.json
    for i, (benchmark_id, benchmark_data) in enumerate(benchmark_clusters.items()):
        # Find match
        match_found = None
        for match in best_matches:
    ...
    # Calculate average deviation for all 15 benchmark clusters from phase4_clusters_refined.json
    deviations = []
    ...
    # Save workbook
    output_path = "output/step2_client_analysis/gemini_2.0_flash_001_step2_analysis_FINAL.xlsx"
    wb.save(output_path)
    
    print(f"✅ Final Step 2 analysis created: {output_path}")
    return output_path

def main():
    """Main function"""
    
    print("🎯 Creating Final Step 2 Analysis")
    print("=" * 60)
    print("This will create a Step 2 analysis that:")
    print("  ✅ Uses phase4_clusters_refined.json as benchmark (15 clusters)")
    ...
    result_path = create_final_step2_analysis()
    
    if result_path:
        print(f"\n🎉 Successfully created final Step 2 analysis!")
        print(f"📁 File saved to: {result_path}")
        print("\n📋 The file includes:")
        print("  ✅ phase4_clusters_refined.json as benchmark (15 clusters)")
        ...
        print("\n❌ Failed to create final Step 2 analysis")
