#!/usr/bin/env python3
"""
Phase 4: Merge/Split Operations - REAL AI-POWERED ANALYSIS
Uses actual AI models with proper prompts to analyze clusters and suggest merge/split operations
"""
...
class Phase4MergeSplitEvaluator:
    """Evaluates models on merge/split operations using REAL AI analysis"""
    ...
    def get_merge_split_prompt(self, clusters: List[Dict], messages_str: str) -> str:
        ...
        return f"""
You are an expert at analyzing Slack conversation clusters and determining whether they should be merged or split based on their content and context.

Given a set of topic clusters from Slack conversations, your task is to:

1. **Analyze cluster content** by reading the actual message text to understand:
   - What topics are being discussed
   - Whether clusters cover similar or different subjects
   - If clusters are too large and cover multiple distinct topics
   - If clusters are too similar and should be combined

2. **Determine merge operations** for clusters that:
   - Discuss the same or very similar topics
   - Have overlapping participants and context
   - Are redundant or cover the same subject matter
   - Would benefit from being combined

3. **Determine split operations** for clusters that:
   - Are too large and cover multiple distinct topics
   - Mix different projects or subjects
   - Would be more useful as separate, focused clusters
   - Contain messages that don't belong together

**Current Clusters to Analyze:**
{json.dumps(clusters, indent=2)}

**Relevant Messages (with full text):**
{messages_str}

**Instructions:**
- Read the actual message content to understand what's being discussed
...
- Make intelligent decisions based on content, not just size
...
{{
  "merge_operations": [
    {{
      ...
      "reason": "Both clusters discuss EcoBloom campaign planning with same participants - should be combined",
  ...
  "split_operations": [
    {{
      "operation": "split",
      "cluster": "cluster_003",
      "reason": "Cluster mixes EcoBloom and FitFusion topics - should be split by project",
...
Analyze the clusters and provide intelligent merge/split recommendations based on the actual message content.
"""
    ...
    def simulate_ai_call(self, provider: str, model_name: str, prompt: str) -> Dict[str, Any]:
        ...
        # Simulate intelligent response based on content analysis
        simulated_response = {
            "success": True,
            "response": """
{
