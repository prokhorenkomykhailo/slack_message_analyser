#!/usr/bin/env python3
"""
Create Step 2 Analysis Using Exact Step 1 Formulas as Google Sheets Formulas
Uses the same formulas from Step 1 but adapted for Step 2 data structure
"""
...
def create_step2_with_step1_formulas():
    """Create Step 2 analysis using exact Step 1 formulas as Google Sheets formulas"""
    
    print("🎯 Creating Step 2 Analysis with Step 1 Formulas as Google Sheets Formulas")
    print("=" * 80)
    ...
    # Summary counts (same format as Step 1)
    ...
    # Calculate metrics using Step 1 approach
    ...
    # Create topic detail rows (same format as Step 1)
    ...
    # Individual topic performance with explicit calculations (same as Step 1)
    for i, (benchmark_id, benchmark_data) in enumerate(benchmark_clusters.items()):
        # Find match
        match_found = None
        for match in best_matches:
    ...
    # Calculate average deviation (same as Step 1)
    deviations = []
    ...
    # Save workbook
    output_path = "output/step2_client_analysis/gemini_2.0_flash_001_step2_analysis_WITH_STEP1_FORMULAS.xlsx"
    wb.save(output_path)
    
    print(f"✅ Step 2 analysis with Step 1 formulas created: {output_path}")
    return output_path

def main():
    """Main function"""
    
    print("🎯 Creating Step 2 Analysis with Step 1 Formulas")
    print("=" * 60)
    ...
    print("  ✅ Uses Google Sheets compatible formulas")
    print("  ✅ Has the same format and structure as Step 1")
    print("  ✅ Shows step-by-step breakdowns")
    print()
    
    # Create the analysis
    result_path = create_step2_with_step1_formulas()
    
    if result_path:
        print(f"\n🎉 Successfully created Step 2 analysis with Step 1 formulas!")
        print(f"📁 File saved to: {result_path}")
        print("\n📋 The file includes:")
        print("  ✅ Exact same formulas as Step 1")
        print("  ✅ Explicit calculations (e.g., '37/66*100=56.06%')")
        print("  ✅ Google Sheets compatible formulas")
