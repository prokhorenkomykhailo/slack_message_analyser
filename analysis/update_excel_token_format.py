#!/usr/bin/env python3
"""
Update Excel files with correct token cost format:
INPUT_TOKENS | OUTPUT_TOKENS | COST_PER_INPUT_TOKEN | COST_PER_OUTPUT_TOKEN | TOKEN_COST
"""
...
def calculate_model_metrics(model_data, benchmark_data, model_name):
    ...
    input_cost_total = cost.get('input_cost', 0)
    output_cost_total = cost.get('output_cost', 0)
    total_cost = cost.get('total_cost', 0)
    
    # Calculate cost per token
    cost_per_input_token = input_cost_total / input_tokens if input_tokens > 0 else 0
    cost_per_output_token = output_cost_total / output_tokens if output_tokens > 0 else 0
...
def create_excel_with_correct_format(metrics, output_dir):
    """Create Excel with exact token cost format"""
    ...
    # TOKEN COST CALCULATION (Exact format from image)
    ...
    token_values = [
        ...
        f"{metrics['COST_PER_INPUT_TOKEN']:.10f}",
        f"{metrics['COST_PER_OUTPUT_TOKEN']:.10f}",
    ...
    # Formula explanation
    ws[f'A{row}'] = "Formula: TOKEN_COST = (INPUT_TOKENS × COST_PER_INPUT_TOKEN) + (OUTPUT_TOKENS × COST_PER_OUTPUT_TOKEN)"
    ...
    for i, result in enumerate(metrics['detailed_results'], 1):
        row_data = [
            i, result['model_title'], result['model_messages'],
            result['benchmark_title'], result['benchmark_messages'],
            result['matched_messages'], result['missing_messages'], result['extra_messages'],
            f"{result['coverage']:.2f}%", f"{result['precision']:.2f}%",
            f"{result['deviation']:.2f}%", "YES" if result['perfect_match'] else "NO"
        ]
    ...
    # Column widths
    widths = {'A': 8, 'B': 30, 'C': 12, 'D': 30, 'E': 12, 'F': 10, 'G': 10, 'H': 10, 'I': 12, 'J': 12, 'K': 12, 'L': 10}
...
def main():
    """Process all models with correct token format"""
    
    print("=== UPDATING EXCEL FILES WITH CORRECT TOKEN FORMAT ===\n")
    ...
    for model_file in model_files:
        ...
        try:
            ...
            if metrics:
                ...
                create_excel_with_correct_format(metrics, output_dir)
        except Exception as e:
            print(f"  ERROR: {e}")
    
    # Create CSV
    print(f"\n=== CREATING CSV WITH CORRECT TOKEN FORMAT ===\n")
    ...
    for m in all_metrics:
        comparison_data.append({
            ...
            'Coverage': round(m['coverage_score'], 2),
            'Precision': round(m['precision_score'], 2),
            'Deviation': round(m['deviation_score'], 2),
            'INPUT_TOKENS': m['INPUT_TOKENS'],
            'OUTPUT_TOKENS': m['OUTPUT_TOKENS'],
            'COST_PER_INPUT_TOKEN': f"{m['COST_PER_INPUT_TOKEN']:.10f}",
            'COST_PER_OUTPUT_TOKEN': f"{m['COST_PER_OUTPUT_TOKEN']:.10f}",
    ...
    df = df[['Rank', 'Model Name', 'Final Score', 'Coverage', 'Precision', 'Deviation',
             'INPUT_TOKENS', 'OUTPUT_TOKENS', 'COST_PER_INPUT_TOKEN', 'COST_PER_OUTPUT_TOKEN',
             'TOKEN_COST', 'Duration (sec)']]
    
    csv_file = f"{output_dir}/step2_all_models_comparison_CORRECT_FORMAT.csv"
    df.to_csv(csv_file, index=False)
    print(f"Created: {csv_file}\n")
    
    print("=== TOP 5 MODELS ===")
    for i, row in df.head(5).iterrows():
        print(f"{row['Rank']}. {row['Model Name']}: {row['Final Score']} (${row['TOKEN_COST']})")
    
    print(f"\n=== COMPLETE ===")
    print(f"Generated {len(all_metrics)} Excel files with CORRECT token format")
