#!/usr/bin/env python3
"""
Create fully explained Step 2 analysis for ALL models with TOKEN USAGE and COST analysis
"""
...
def calculate_model_metrics(model_data, benchmark_data, model_name):
    """Calculate metrics for a single model including token usage"""
    ...
    cost_per_input_token = (input_cost / prompt_tokens * 1000000) if prompt_tokens > 0 else 0  # Cost per 1M tokens
    cost_per_output_token = (output_cost / completion_tokens * 1000000) if completion_tokens > 0 else 0  # Cost per 1M tokens
...
def create_excel_for_model(metrics, output_dir):
    """Create Excel file for a single model with token usage"""
    ...
    token_data = [
        ...
        ["Input Tokens (Prompt)", f"{metrics['prompt_tokens']:,}", "Tokens in the prompt sent to the model"],
        ["Output Tokens (Completion)", f"{metrics['completion_tokens']:,}", "Tokens generated by the model"],
        ["Total Tokens", f"{metrics['total_tokens']:,}", "Total tokens used (Input + Output)"],
        ["", "", ""],
        ["Input Cost", f"${metrics['input_cost']:.6f}", "Cost for input tokens"],
        ["Output Cost", f"${metrics['output_cost']:.6f}", "Cost for output tokens"],
        ["Total Cost", f"${metrics['total_cost']:.6f}", "Total cost for this request"],
        ["", "", ""],
        ["Cost per 1M Input Tokens", f"${metrics['cost_per_input_token']:.2f}", "Price per million input tokens"],
        ["Cost per 1M Output Tokens", f"${metrics['cost_per_output_token']:.2f}", "Price per million output tokens"],
        ["", "", ""],
        ["Tokens per Second", f"{metrics['total_tokens']/metrics['duration']:.2f}" if metrics['duration'] > 0 else "N/A", "Token generation speed"],
        ["Cost per Second", f"${metrics['total_cost']/metrics['duration']:.6f}" if metrics['duration'] > 0 else "N/A", "Cost efficiency"]
    ]
    ...
    for i, item in enumerate(token_data):
        ...
        ws[f'B{row}'] = item[1]
        ws[f'C{row}'] = item[2]
        ws[f'A{row}'].font = subheader_font if i == 0 else data_font
        ws[f'B{row}'].font = subheader_font if i == 0 else data_font
        ws[f'C{row}'].font = subheader_font if i == 0 else data_font
        ...
        ws[f'B{row}'].alignment = center_alignment if i > 0 else left_alignment
...
def main():
    ...
    print("=== CREATING STEP 2 ANALYSES FOR ALL MODELS (WITH TOKEN USAGE) ===\n")
    ...
    # Create comparison CSV with token usage
    print(f"\n=== CREATING COMPARISON CSV (WITH TOKEN USAGE) ===\n")
    ...
    for m in all_metrics:
        comparison_data.append({
            ...
            'Input Tokens': m['prompt_tokens'],
            'Output Tokens': m['completion_tokens'],
            'Total Tokens': m['total_tokens'],
            'Input Cost ($)': round(m['input_cost'], 6),
            'Output Cost ($)': round(m['output_cost'], 6),
            'Total Cost ($)': round(m['total_cost'], 6),
            'Cost per 1M Input Tokens ($)': round(m['cost_per_input_token'], 2),
            'Cost per 1M Output Tokens ($)': round(m['cost_per_output_token'], 2),
            'Tokens per Second': round(m['total_tokens']/m['duration'], 2) if m['duration'] > 0 else 0,
            'Success': m['success']
        })
    ...
    df = df[['Rank', 'Model Name', 'Final Score', 'Model Clusters', 'Coverage Score', 
             ...
             'Split Operations', 'Duration (sec)', 'Input Tokens', 'Output Tokens', 
             'Total Tokens', 'Input Cost ($)', 'Output Cost ($)', 'Total Cost ($)',
             'Cost per 1M Input Tokens ($)', 'Cost per 1M Output Tokens ($)', 
             'Tokens per Second', 'Success']]
    
    # Save CSV
    csv_file = f"{output_dir}/step2_all_models_comparison_with_tokens.csv"
    ...
    for i, row in df.head(5).iterrows():
        print(f"{row['Rank']}. {row['Model Name']}: {row['Final Score']} (${row['Total Cost ($)']:.6f}, {row['Total Tokens']} tokens)")
    
    # Print top 5 by Cost Efficiency (lowest cost)
    print("\n=== TOP 5 MOST COST-EFFICIENT MODELS ===")
    df_cost = df.sort_values('Total Cost ($)')
    for i, row in df_cost.head(5).iterrows():
        print(f"{i+1}. {row['Model Name']}: ${row['Total Cost ($)']:.6f} (Score: {row['Final Score']})")
    
    print(f"\n=== COMPLETE ===")
    print(f"Generated {len(all_metrics)} Excel files with token usage")
    print(f"Generated 1 comparison CSV with token usage")
