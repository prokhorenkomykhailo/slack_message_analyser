#!/usr/bin/env python3
"""
Complete Step 2 Analysis for Google Gemini 2.0 Flash 001
Compares balanced refinement results with benchmark clusters
Includes all detailed sections like Step 1 analysis
"""
...
def calculate_detailed_metrics(model_data, benchmark_data):
    """Calculate detailed metrics comparing model output with benchmark"""
    ...
    total_messages = len(model_message_map)
    ...
    # Check each message
    for msg_id in range(1, 301):  # Assuming 300 messages total
    ...
    coverage_percentage = (matched_messages / total_messages) * 100 if total_messages > 0 else 0
    ...
    # Quality Rate
    quality_rate = (matched_messages / total_messages) * 100 if total_messages > 0 else 0
...
def create_complete_step2_analysis():
    """Create the complete Step 2 analysis Excel file"""
    ...
    # Calculate metrics
    metrics = calculate_detailed_metrics(model_data, benchmark_data)
    cluster_comparison = create_detailed_cluster_comparison(model_data, benchmark_data)
    
    # Create workbook
    wb = Workbook()
    ws = wb.active
    ws.title = "Step 2 Complete Analysis"
    ...
    ws[f'A{row}'] = "Step 2 Analysis: Google Gemini 2.0 Flash 001 - Balanced Refinement Results"
    ...
    model_info = [
        ["Model Provider", model_data.get('provider', 'N/A')],
        ["Model Name", model_data.get('model', 'N/A')],
        ["Phase", model_data.get('phase', 'N/A')],
        ["Timestamp", model_data.get('timestamp', 'N/A')],
        ["Success", model_data.get('success', 'N/A')],
        ["Duration (seconds)", model_data.get('duration', 'N/A')],
        ["Step 1 Clusters", model_data.get('metrics', {}).get('step1_clusters', 'N/A')],
        ["Refined Clusters", model_data.get('metrics', {}).get('refined_clusters', 'N/A')],
        ["Merge Operations", model_data.get('metrics', {}).get('merge_operations', 'N/A')],
        ["Split Operations", model_data.get('metrics', {}).get('split_operations', 'N/A')],
        ["Cluster Change", model_data.get('metrics', {}).get('cluster_change', 'N/A')],
        ["Target Met", model_data.get('metrics', {}).get('target_met', 'N/A')],
        ["Target Range", model_data.get('metrics', {}).get('target_range', 'N/A')],
        ["AI Claimed Count", model_data.get('metrics', {}).get('ai_claimed_count', 'N/A')]
    ]
    ...
    ws[f'A{row}'] = "Performance Metrics (Step 1 Formulas)"
    ws[f'A{row}'].font = subheader_font
    ws[f'A{row}'].fill = subheader_fill
    ws.merge_cells(f'A{row}:H{row}')
    row += 1
    ...
    metrics_data = [
        ...
        ["Total Messages", metrics['total_messages'], "=COUNT(Benchmark!B:B)-1", "Total number of messages in dataset"],
        ["Model Cluster Count", metrics['model_cluster_count'], "=COUNTA(Model_Output!A:A)-1", "Number of clusters in model output"],
        ["Benchmark Cluster Count", metrics['benchmark_cluster_count'], "=COUNTA(Benchmark!A:A)-1", "Number of clusters in benchmark"],
        ["Matched Messages", metrics['matched_messages'], "=SUMPRODUCT((Model_Output!B:B=Benchmark!B:B)*1)", "Messages correctly placed"],
        ["Missing Messages", metrics['missing_messages'], "=Benchmark_Count-Matched_Count", "Messages missing from model"],
        ["Extra Messages", metrics['extra_messages'], "=Model_Count-Matched_Count", "Extra messages in model"],
        ["Coverage %", f"={metrics['coverage_percentage']:.2f}%", "=Matched_Count/Total_Messages*100", "Percentage of messages covered"],
        ["Precision %", f"={metrics['precision_percent']:.2f}%", "=Matched_Count/(Matched_Count+Extra_Count)*100", "Accuracy of model clusters"],
        ["Recall %", f"={metrics['recall_percent']:.2f}%", "=Matched_Count/(Matched_Count+Missing_Count)*100", "Completeness of model clusters"],
        ["Message Count Deviation", metrics['message_count_deviation'], "=ABS(Model_Count-Benchmark_Count)", "Difference in total messages"],
        ["Message Count Deviation %", f"={metrics['message_count_deviation_percent']:.2f}%", "=Message_Deviation/Benchmark_Count*100", "Percentage deviation"],
        ...
        ["Quality Rate", f"={metrics['quality_rate']:.2f}%", "=Matched_Count/Total_Messages*100", "Quality of clustering"],
        ["Operation Efficiency", f"={metrics['operation_efficiency']:.2f}%", "=Total_Operations/Model_Count*100", "Efficiency of merge/split operations"],
        ["Size Optimization", f"={metrics['size_optimization']:.2f}%", "=100-ABS(Model_Avg_Size-Benchmark_Avg_Size)/Benchmark_Avg_Size*100", "How well cluster sizes are optimized"]
    ]
    ...
    # Save the file
    output_file = "output/step2_client_analysis/gemini_2.0_flash_001_step2_analysis_COMPLETE.xlsx"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    wb.save(output_file)
    
    print(f"Complete Step 2 analysis saved to: {output_file}")
...
if __name__ == "__main__":
    create_complete_step2_analysis()
