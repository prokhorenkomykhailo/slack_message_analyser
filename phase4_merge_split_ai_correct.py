#!/usr/bin/env python3
"""
Phase 4: Merge/Split Operations - CORRECT AI-POWERED ANALYSIS
Uses AI models to analyze message content and cosine similarity for intelligent merge/split operations
"""
...
"""
    ...
    def calculate_cosine_similarity(self, cluster1: Dict, cluster2: Dict) -> float:
        """Calculate cosine similarity between two clusters based on message content"""
        try:
    ...
    def analyze_cluster_coherence(self, cluster: Dict) -> float:
        ...
        try:
            ...
            np.fill_diagonal(similarity_matrix, 0)  # Remove diagonal (self-similarity)
            ...
            "response": """
{
  ...
  "refined_clusters": [
    ...
    def evaluate_model(self, provider: str, model_name: str, phase3_result: Dict) -> Dict[str, Any]:
        ...
        # Calculate cosine similarities between clusters
        print(f"    🔍 Calculating cosine similarities between clusters...")
        cluster_similarities = []
        for i, cluster1 in enumerate(clusters):
            for j, cluster2 in enumerate(clusters[i+1:], i+1):
                similarity = self.calculate_cosine_similarity(cluster1, cluster2)
                cluster_similarities.append({
                    'cluster1': cluster1['cluster_id'],
                    'cluster2': cluster2['cluster_id'],
                    'similarity': similarity
                })
                print(f"      {cluster1['cluster_id']} vs {cluster2['cluster_id']}: {similarity:.3f}")
